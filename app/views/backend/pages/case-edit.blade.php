...
<h1>
  编辑案例
  <a href="{{url('/admin/case/all')}}" class="btn btn-info">案例列表</a>
</h1>
@endsection
@section('content')
<div class="row">
  <form action="{{url('/admin/case/update', array('id' =>
    $cases->id))}}" method="post" accept-charset="utf-8" class="form-horizontal">
    <div class="col-md-8">
      ...
      <div class="form-group">
        ...
        <div class="col-md-10">
          <input type="text" class="form-control " name="title" value="{{Input::old('title', $cases->case_title)}}" placeholder="案例标题">
          <br></div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="col-md-10">
          <input type="text" class="form-control " name="client_name" value="{{Input::old('client_name', $cases->client_name)}}" placeholder="客户名称">
          <br></div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="col-md-10">
          <input type="text" class="form-control " name="service_content" value="{{Input::old('service_content', $cases->service_content)}}" placeholder="服务内容，多个内容以空格分隔">
          <br></div>
      </div>
      <div class="form-group">
        ...
        <div class="col-md-3">
          <select name="industry" class="selectboxit col-md-3">
            ...
            <option value="{{ $industry['id'] }}" @if(Input::old('industry', $cases->case_industry) == $industry['id'])selected@endif>{{ $industry['name'] }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="col-md-3">
          <select name="specialty" class="selectboxit col-md-3">
            ...
            <option value="{{ $specialty['id'] }}" @if(Input::old('specialty', $cases->case_specialty) == $specialty['id'])selected@endif>{{ $specialty['name'] }}</option>
            @endforeach
          </select>
        </div>
      </div>
      <div class="form-group">
        ...
        <div class="col-md-10">
          <script id="myEditor" name="desc" type="text/plain" style="width:100%;height:300px;">{{Input::old('desc', $cases->case_desc)}}</script>
        </div>
      </div>
          ...
          <div class="form-group img-div">
            ...
            <div id="uploader-demo">
                ...
                <ul id="fileList" class="uploader-list">
                  <?php $images = Input::old('images', $cases->images->lists('src')); ?>
                  @if(!empty($images))
...
<script src="{{asset('/backend/js/fileuploader.js')}}" type="text/javascript" ></script>
  <script type="text/javascript">var editor = UM.getEditor('myEditor')</script>
  @endsection